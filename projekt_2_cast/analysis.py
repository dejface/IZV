...
def get_dataframe(filename: str, verbose: bool = False) -> pd.DataFrame:
    ...
    floats = [44,45,46,47,48,49]
    strs = [50,51,52,53,54,55,56,57,58,61]
...
def plot_conseq(df: pd.DataFrame, fig_location: str = None,
    ...
    deaths_per_region = df.groupby('region', as_index=False).agg({'p13a':'sum'}).reset_index().sort_values('p13a',ascending=False)
    heavy_injuries = df.groupby('region', as_index=False).agg({'p13b':'sum'}).reset_index().sort_values('p13b',ascending=False)
    light_injuries = df.groupby('region', as_index=False).agg({'p13c':'sum'}).reset_index().sort_values('p13c',ascending=False)
    accidents_count = df.groupby('region', as_index=False).agg({'p1':'count'}).reset_index().sort_values('p1',ascending=False)

    fig, (ax1, ax2, ax3, ax4) = plt.subplots(4, 1, figsize=(11.69,8.27))
    fig.suptitle("Number of accidents in Czech regions through years 2016-2020",fontsize=12)
    fig.subplots_adjust(hspace=0.7, top=0.91, bottom=0.05)
    fig.patch.set_facecolor('#C0C0C0')

    #deaths plot
    graph1 = sns.barplot(data=deaths_per_region, y=deaths_per_region['p13a'], x=deaths_per_region['region'], ax=ax1, color='#CC00CC')
    ...
    graph1.grid(color='#95a5a6', linestyle='-', linewidth=1, axis='y', alpha=0.3)
    graph1.set_facecolor('#FFCCFF')
    count = 0
    for index, row in deaths_per_region.iterrows():
        graph1.text(count, 0, row.p13a, color='black', ha="center", fontsize=8)
        count += 1

    #heavy injuries plot
    graph2 = sns.barplot(data=heavy_injuries, y=heavy_injuries['p13b'], x=heavy_injuries['region'], ax=ax2, color='#3399FF')
    ...
    graph2.grid(color='#95a5a6', linestyle='-', linewidth=1, axis='y', alpha=0.3)
    graph2.set_facecolor('#CCCCFF')
    count = 0
    for index, row in heavy_injuries.iterrows():
        graph2.text(count, 0, row.p13b, color='black', ha="center",fontsize=8)
        count += 1

    #light injuries plot
    graph3 = sns.barplot(data=light_injuries,y=light_injuries['p13c'], x=light_injuries['region'], ax=ax3, color='#006633')
    ...
    graph3.grid(color='#95a5a6', linestyle='-', linewidth=1, axis='y', alpha=0.3)
    graph3.set_facecolor('#CCFFE5')
    count = 0
    for index, row in light_injuries.iterrows():
        graph3.text(count, 0, row.p13c, color='black', ha="center",fontsize=8)
        count += 1
    
    #total accidents plot
    graph4 = sns.barplot(data=accidents_count, y=accidents_count['p1'], x=accidents_count['region'], ax=ax4, color='#FF0000')
    ...
    graph4.grid(color='#95a5a6', linestyle='-', linewidth=1, axis='y', alpha=0.3)
    graph4.set_facecolor('#FFCCCC')
    count = 0
    for index, row in accidents_count.iterrows():
    ...
    if fig_location is not None:
        fig.savefig(os.path.join(os.path.abspath(os.getcwd()),fig_location),dpi=600)
...
def plot_damage(df: pd.DataFrame, fig_location: str = None,
    ...
    new_df['Damage cause'] = pd.cut(x=new_df['p12'],bins=[100,201,301,401,501,601,615], right=False,\
        labels=['wasn\'t caused by driver','excessive speed','incorrect overtaking',\
        'wasn\'t given priority in driving', 'incorrect way of driving','technical problem on a vehicle'])
    
    new_df['damage'] = pd.cut(x=new_df['p53'],bins=[0,500,2000,5000,10000,np.inf], right=False,\
        labels=['<50','50 - 200','200 - 500',\
        '500 - 1000', '>1000'])

    new_df['count'] = 0
    df_final = new_df.groupby(['region','damage', 'Damage cause'], as_index=False)['p1'].agg(['count']).reset_index()

    graph1 = sns.catplot(x='damage', col='region', y='count', data=df_final, kind='bar', hue='Damage cause', col_wrap=2, ci=None, sharex=False)
    graph1.set_axis_labels("Damage [thousands CZK]", "Count")
    graph1.fig.get_axes()[0].set_yscale('log')
    graph1.fig.suptitle('Causes of accidents and their damage [thousands CZK] through years 2016-2020',fontsize=11)
    ...
    for i in range(4):
        ...
        axes[i].grid(color='#95a5a6', linestyle='-', linewidth=1, axis='y', alpha=0.3)
        axes[i].set_facecolor('#FFCCFF')

    if show_figure:
        plt.show()

    if fig_location is not None:
        graph1.fig.savefig(os.path.join(os.path.abspath(os.getcwd()),fig_location),dpi=600)
